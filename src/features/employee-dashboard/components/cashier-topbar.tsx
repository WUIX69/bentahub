...
export function EmployeeTopbar({ onToggleSidebar }: EmployeeTopbarProps) {
  ...
  if (pathname === "/employee/stock-check") {
    title = "Stock Check"
  } else if (pathname === "/employee/payments") {
    title = "Validate Payments"
  } else if (pathname === "/employee/transactions") {
    title = "Transaction History"
  } else if (pathname === "/employee/notifications") {
    title = "Notifications"
  }
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-4 md:px-6 sticky top-0 z-30 flex justify-between items-center h-[80px] w-full">
      ...
      <div className="flex items-center gap-4 md:gap-6">
        {/* Notification Bell */}
        <Link href="/employee/notifications" className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-500 hover:text-primary transition-colors border border-slate-200 relative flex-shrink-0">
